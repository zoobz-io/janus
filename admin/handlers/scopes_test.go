...
// TestScopeEndpointsEnforceScope verifies each endpoint denies a caller that
...
func TestScopeEndpointsEnforceScope(t *testing.T) {
	...
	roccotest.AssertStatus(t, roccotest.ServeRequest(reads, "GET", "/applications/a1/scopes", nil), http.StatusForbidden)
	...
	roccotest.AssertStatus(t, roccotest.ServeRequest(writes, "POST", "/applications/a1/scopes", nil), http.StatusForbidden)
	roccotest.AssertStatus(t, roccotest.ServeRequest(writes, "DELETE", "/applications/a1/scopes/s1", nil), http.StatusForbidden)
}
