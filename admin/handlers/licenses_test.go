...
// TestLicenseEndpointsEnforceScope verifies each endpoint denies a caller that
...
func TestLicenseEndpointsEnforceScope(t *testing.T) {
	...
	roccotest.AssertStatus(t, roccotest.ServeRequest(reads, "GET", "/applications/a1/licenses", nil), http.StatusForbidden)
	...
	roccotest.AssertStatus(t, roccotest.ServeRequest(writes, "POST", "/applications/a1/licenses", nil), http.StatusForbidden)
	roccotest.AssertStatus(t, roccotest.ServeRequest(writes, "DELETE", "/applications/a1/licenses/tn1", nil), http.StatusForbidden)
}
